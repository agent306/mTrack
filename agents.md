...
`Customer/WorkspaceController.php` and `Pages/Customer/Workspace.vue` implement customer onboarding, SMS setup, proof-backed discovery and claiming, saved dashboard widgets, live maps, circle/polygon places, crossing reports with CSV export, and journey history. `CustomerShell.vue` provides the shared responsive light/dark shell. Legacy portal pages remain for account administration and scoped-role compatibility.
